...
Shader "Laya/Legacy/Sky Panoramic" {
Properties {
    tintColor ("Tint Color", Color) = (.5, .5, .5, .5)
    [Gamma] exposure ("Exposure", Range(0, 8)) = 1.0
    rotation ("Rotation", Range(0, 360)) = 0
    [NoScaleOffset] panoramicTexture ("Spherical  (HDR)", 2D) = "grey" {}
...
SubShader {
    ...
    Pass {
        ...
        sampler2D panoramicTexture;
        float4 panoramicTexture_TexelSize;
        half4 panoramicTexture_HDR;
        half4 tintColor;
        half exposure;
        float rotation;
...
/*#ifdef _MAPPING_6_FRAMES_LAYOUT
        ...
        v2f vert (appdata_t v)
        {
           ...
           // float sourceAspect = float(panoramicTexture_TexelSize.z) / float(panoramicTexture_TexelSize.w);
           ...
           // o.edgeSize.xy =panoramicTexture_TexelSize.xy * 0.5 / o.layout.yz - 0.5;
        ...
        fixed4 frag (v2f i) : SV_Target
        {
            ...
            half4 tex = tex2D (panoramicTexture, tc);
            half3 c = DecodeHDR (tex, panoramicTexture_HDR);
            c = c * tintColor.rgb * unity_ColorSpaceDouble.rgb;
            c *= exposure;
            return half4(c, 1);
        }
        ENDCG
...
CustomEditor "LayaSkyboxPanoramicShaderGUI"
Fallback Off
