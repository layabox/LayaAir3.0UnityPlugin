...
Properties {
    ...
    sunSizeConvergence("Sun Size Convergence", Range(1,10)) = 5

    atmosphereThickness ("Atmosphere Thickness", Range(0,5)) = 1.0
    skyTint ("Sky Tint", Color) = (.5, .5, .5, 1)
    groundTint ("Ground", Color) = (.369, .349, .341, 1)

    exposure("Exposure", Range(0, 8)) = 1.3
}
...
SubShader {
    ...
    Pass {
        ...
        #pragma multi_compile SUNDISK_NONE SUNDISK_SIMPLE SUNDISK_HIGH_QUALITY

        uniform half exposure;     // HDR exposure
        uniform half3 groundTint;
        uniform half sunSize;
        uniform half sunSizeConvergence;
        uniform half3 skyTint;
        uniform half atmosphereThickness;
        ...
        v2f vert (appdata_t v)
        {
            ...
            float3 kSkyTintInGammaSpace = COLOR_2_GAMMA(skyTint); // convert tint from Linear back to Gamma
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
            ...
            // 1. in case of linear: multiply by exposure in here (even in case of lerp it will be common multiplier, so we can skip mul in fshader)
            // 2. in case of gamma and SKYBOX_COLOR_IN_TARGET_COLOR_SPACE: do sqrt right away instead of doing that in fshader

            OUT.groundColor = exposure * (cIn + COLOR_2_LINEAR(groundTint) * cOut);
            OUT.skyColor    = exposure * (cIn * getRayleighPhase(_WorldSpaceLightPos0.xyz, -eyeRay));
...
Fallback Off
CustomEditor "LayaSkyboxProceduralShaderGUI"
}
